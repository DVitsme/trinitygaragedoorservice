...
/*
  The `Ph` placeholder highlighter was removed 2026-08-04, along with its last two users. It existed
  to make unconfirmed client facts visibly red so they could not ship unnoticed (convention G16), and
  it did its job everywhere except here, where the page went live on 1 August with the highlights
  still in it. If a future page needs the same device, bring it back, but treat a highlight that
  reaches production as a bug rather than a note.
*/
      ...
      <section className="border-b-[5px] border-accent bg-[#161616] px-6 py-16">
        <div className="mx-auto max-w-[860px]">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            {/*
              1 August 2026 is the day the site went live at trinitygaragedoorservice.com, which is
              the first day this policy governed anyone's real visit. Filled 2026-08-04; it read
              "[date you publish]" on the live page until then.

              ⚠️ Hardcoded, NOT computed from `new Date()`. An effective date that silently follows
              the build clock would claim the policy changed on every deploy, and would make it
              impossible to say afterwards which text someone actually agreed to. Change it by hand,
              and only when the policy's substance changes.
            */}
            <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/[0.08] px-[15px] py-2 text-[13.5px] font-semibold text-white">
              Effective date: <span className="font-bold text-white">1 August 2026</span>
            </span>
          </div>
        </div>
      </section>
...
{/*
            REMOVED 2026-08-04: a red callout reading "Starting template tailored to what this site
            collects. Have it reviewed by legal counsel before publishing, and fill the highlighted
            placeholders..."

            It was a note to ourselves and it went live on 1 August, so for three days a customer
            reading the privacy policy was told by the policy that it was an unreviewed template with
            blanks in it. That is worse than the blanks were: it invites exactly the "you knew it was
            inadequate" reading that FTC §5 and FDUTPA turn on, and it is the opposite of the reason
            this page was rewritten when tracking shipped.

            The placeholders it referred to are now filled. **The legal review it asked for is still
            genuinely outstanding** (`CLIENT-ASKS` #12, `LAUNCH-TODO` 2.7), it just belongs in our
            docs rather than on the page a customer reads.
          */}
          ...
          <div className="mt-11 rounded-[12px] bg-ink p-[30px_32px] text-white">
            ...
            <div className="flex flex-col gap-[9px] text-[15.5px] text-[#e8e8e8]">
              ...
              {/*
                Filled 2026-08-04. Both were visible placeholders on the LIVE page until then, which
                undercut the point of the policy: a legal page reading "[contact email to confirm]"
                does not look like a document anyone stands behind.

                The address is composed from `SITE.address` rather than retyped, so it can never
                drift from the footer or the JSON-LD. It is the Housecall Pro company record, the
                address they actually dispatch from, verified 2026-07-28.

                The email is the office inbox that already receives every lead, so a privacy request
                lands in front of the same people, with no new mailbox for anyone to remember to
                check.
              */}
              <div><span className="text-[#9a9a9a]">Email:</span> <a href="mailto:trinitygaragedoorservice@gmail.com" className="font-bold text-white no-underline">trinitygaragedoorservice@gmail.com</a></div>
              <div><span className="text-[#9a9a9a]">Address:</span> {SITE.address.street}, {SITE.address.city}, {SITE.address.region} {SITE.address.postalCode}</div>
