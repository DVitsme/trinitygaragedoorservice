...
/**
 * The shared body of every request form page.
 *
 * Extracted from `app/get-service/page.tsx` on 2026-08-04, when Housecall Pro's booking modal was
 * switched off and its CTAs were repointed at our own form. The site now has several form pages
 * rather than one (see `lib/booking.ts` for why), and they have to stay identical below the hero:
 * the same fields, the same trust, the same phone panel. Only the words at the top change.
 *
 * ⚠️ **This stays a real page and does NOT become a modal**, despite the ads specialist asking for a
 * popup. A modal has no URL, and Google Ads' Landing Page report and Quality Score are evaluated per
 * URL, so a modal only form is invisible to exactly the attribution this rebuild exists to provide.
 * What he actually asked for was something nicer than the Housecall Pro popup, and the answer to
 * that is to make the page nicer, which is what this is.
 */
const reasons = [
...
export type RequestFormLayoutProps = {
  /** Small uppercase kicker above the h1, and the breadcrumb leaf. */
  eyebrow: string;
  h1: string;
  blurb: string;
  /** Retitles the form for the free estimate variant. Only `/get-service/?intent=estimate` sets it. */
  intent?: string;
  /**
   * Which door this lead came in by. Written to the `source` column, shown on the lead email, and
   * sent as `lead_source` on `generate_lead`. Leave unset on `/get-service/` so it keeps emitting
   * the `contact-form` / `estimate-form` values that already exist in the client's data.
   */
  source?: LeadSource;
};

export function RequestFormLayout({ eyebrow, h1, blurb, intent, source }: RequestFormLayoutProps) {
  return (
    <>
      {/*
        ...
        */}
        ...
        <div className="relative z-[2] mx-auto max-w-[1200px] px-5 py-[92px] max-nav:py-[68px] nav:px-8">
          ...
          <div className="mt-4 flex items-center gap-3.5">
            ...
            <span className="text-[13px] font-extrabold uppercase tracking-[0.22em] text-white">{eyebrow}</span>
          </div>
          <h1 className="m-0 mt-[18px] max-w-[820px] font-display text-[clamp(30px,4.8vw,54px)] font-black uppercase leading-[1.0] text-white">
            {h1}
          </h1>
          <p className="mt-5 max-w-[620px] text-[clamp(16.5px,2vw,19px)] leading-[1.58] text-[#a8a8a8]">{blurb}</p>
        </div>
      </section>
