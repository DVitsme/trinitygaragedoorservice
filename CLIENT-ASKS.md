...
**Living document.** Last updated: 2026-07-28. Deeper detail: `PRE-LAUNCH-PUNCHLIST.md`,
...
|---|---|---|---|
| 1 | **Which phone number is the main one?** | **Nearly settled.** Their own Housecall Pro account lists **(813) 279-6785** as the company number, which is what the site already uses everywhere. Contact page lists all three county lines. Just needs a yes from Jason. | ◐ |
...
| 4 | 🔴 **What are the real office hours?** | **Now four different answers, and we left all of them alone on purpose.** Google says Mon to Sat 7am to 9pm. Their front door says Mon to Fri 8am to 5:30pm. **Their Housecall Pro booking system says Mon to Fri 8am to 4pm, closed weekends**, and that is the one that actually decides when a customer can book online. Which is right? | ☐ |
| 4b | 🔴 **Is the "24/7" promise deliberate, and is somebody actually answering?** | **This is a strategy question, not a bug report, which is why we have changed nothing.** The site promises 24/7 in about 30 places, and the website code separately tells Google the business is **open 24 hours a day, 7 days a week**. Their booking system says weekdays 8 to 4. Those can both be true if the phone genuinely rings through at 2am on a Sunday. **So: does it?** If yes, we leave it and say so proudly. If it really means "we try hard", we soften the wording. The reason to ask rather than assume: a customer who calls at 2am and gets voicemail has a screenshot and a grievance, and one of these promises sits inside the FAQ data Google can republish. | ☐ |
...
| 5b | 🔴 **Which counties are you licensed in, and is the Pasco licence renewed?** | The site says "licensed in **Florida** under GD13010 and GDI-09484" on **25 pages**. Those look like **county** licences (Hillsborough and Pasco), not a Florida state licence, and the Pasco one looks like it **expires 30 Sept 2026**. Two questions: (1) do you hold a Florida state licence, or are these county ones? (2) is the Pasco renewal filed? **This is the one item on the whole list with legal exposure, so we should not guess.** Also: we now advertise 5 counties of coverage, so if the licences cover 2, we need to know. | ☐ |
| 6 | **A public email address** | Housecall Pro has **trinitygaragedoorservice@gmail.com** as their support email. We can use it, but a proper address at their own domain would look far better. Which do they want? | ◐ |
...
| # | Ask | Why it matters | Status |
|---|---|---|---|
| 29 | **Is the "Drive Social" API key set to Full access or Read-only?** | It is in Housecall Pro under **My Apps → API Key Management**, and the setting is shown next to the key. See the security note in `CLIENT-NOTES.md` before raising this. **Ask gently, this is hygiene not an accusation.** | ☐ |
| 30 | **Is Drive Social Media still an active vendor?** | If not, that key should be deleted. Access does not expire on its own. | ☐ |
...
| 36 | 🔑 **Can we turn on website analytics? (recommend yes)** | **The site has no analytics at all today, so nobody can tell whether it is working.** We recommend **Cloudflare Web Analytics**: free, already part of the hosting they use, no cookies, no consent banner, and no effect on speed. It is what makes #35 above produce an actual number instead of a rough guess. Two things come with it: we need **Cloudflare account access** (same as #7), and we update the privacy policy the same day, because it currently promises we will. If they would rather have Google Analytics we can do that instead, but it is heavier, uses cookies, and brings consent questions this one avoids. | ☐ |
